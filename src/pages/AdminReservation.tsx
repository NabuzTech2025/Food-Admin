...
import { 
  Calendar, Users, Clock3, Phone, UserRound, Search, CheckCircle2, Eye, CalendarCheck, Utensils
} from "lucide-react";
import { filterStoreReservations, type ReservationDetail } from "../api/reservation";
...
const AdminReservation = () => {
  ...
  const fetchReservations = async () => {
    ...
    try {
      setLoading(true);
      setError(null);
      const res = await filterStoreReservations({
  ...
  const getStatusBadge = (status: string) => {
    switch(status.toLowerCase()) {
      case 'booked':
        return <Badge className="bg-green-500 hover:bg-green-600 text-white flex items-center gap-1"><CheckCircle2 className="w-3 h-3"/> BOOKED</Badge>;
      case 'pending':
        return <Badge className="bg-orange-500 hover:bg-orange-600 text-white flex items-center gap-1"><Eye className="w-3 h-3"/> PENDING</Badge>;
      case 'cancelled':
      ...
      <Tabs value={activeTab} onValueChange={setActiveTab} className="w-full">
        <div className="flex flex-col md:flex-row md:items-center justify-between gap-4 mb-6">
          <TabsList className="h-11">
            ...
            <TabsTrigger value="today_received_booking" className="text-sm px-6">
          ...
          <div className="text-sm text-muted-foreground font-medium flex items-center gap-2 bg-muted/50 px-4 py-2 rounded-lg">
            ...
            Total Reservations: <span className="text-foreground font-bold">{filteredData.length}</span>
          </div>
        </div>
        ...
        <TabsContent value="today_booking" className="m-0">
          {loading ? (
            ...
            <div className="py-16 flex flex-col items-center justify-center text-center border-2 border-dashed rounded-xl bg-muted/20">
              ...
              <p className="text-muted-foreground">Try selecting a different date or search term.</p>
            </div>
          ) : (
            <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 gap-4 pb-12">
              {filteredData.map((res) => (
                <Card key={res.id} className="overflow-hidden hover:shadow-md transition-shadow bg-card">
                  <CardContent className="p-4 flex flex-col gap-4">
                    ...
                    <div className="flex justify-between items-center pl-[40px]">
                      <div className="font-medium text-foreground text-sm truncate pr-2">
                        {res.customer_name}
                        {res.customer_phone && <span className="text-muted-foreground font-normal">/{res.customer_phone}</span>}
                      </div>
                    ...
                    <div className="flex justify-between items-center pl-[40px] mt-1">
                      <div className="flex items-center gap-2">
                        ...
                        <span className="font-bold text-foreground text-base">{res.guest_count}</span>
                      </div>
                      <div className="flex items-center gap-2">
        ...
        <TabsContent value="today_received_booking">
          <div className="py-16 flex flex-col items-center justify-center text-center border-2 border-dashed rounded-xl bg-muted/20">
            <h3 className="text-lg font-medium">Coming Soon</h3>
            <p className="text-muted-foreground">Today received booking API integration will be here.</p>
          </div>
        </TabsContent>
      </Tabs>
