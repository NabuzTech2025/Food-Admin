...
import { useGetPostcodes, useDeletePostcode } from "@/hooks/usePostcode";
...
function PostcodePage() {
  ...
  const [editingItem, setEditingItem] = useState<Postcode | null>(null);
  const [deleteTarget, setDeleteTarget] = useState<Postcode | null>(null);

  // ── Data ──
  const { data: postcodesRaw, isLoading } = useGetPostcodes(store_id);
  const { mutate: deleteItem, isPending: isDeleting } = useDeletePostcode();

  const postcodes = Array.isArray(postcodesRaw) ? postcodesRaw : [];

  // ── Search filter ──
  const filtered = useMemo(() => {
    const q = search.toLowerCase();
    if (!q) return postcodes;
    return postcodes.filter((p) => p.postcode.toLowerCase().includes(q));
  }, [postcodes, search]);
  ...
  const openEditForm = (item: Postcode) => {
    setEditingItem(item);
    setFormOpen(true);
  };
  ...
  return (
    <div className="space-y-4">
      <div className="bg-white rounded-xl border border-border shadow-sm flex flex-col h-[calc(100vh-80px)]">
                ...
                <div className="flex items-start justify-between gap-3">
                  <div>
                    ...
                    <p className="text-xs text-neutral-500 mt-0.5">
                      Min: {item.minimum_order_amount} · Fee:{" "}
                      {item.delivery_fee} · {item.delivery_time} min
                    </p>
                  </div>
        ...
        <div className="flex-shrink-0 px-4 py-2 border-t bg-muted/30">
          <p className="text-xs text-neutral-500">
            ...
            {search && ` (filtered from ${postcodes.length} total)`}
          </p>
        </div>
      </div>
      ...
      >
        <AlertDialogContent>
          <AlertDialogHeader>
            <AlertDialogTitle>Delete Postcode?</AlertDialogTitle>
            <AlertDialogDescription>
              Postcode{" "}
              <span className="font-medium text-neutral-800">
                {deleteTarget?.postcode}
              </span>{" "}
              permanently delete ho jaega. Yeh action undo nahi ho sakta.
            </AlertDialogDescription>
          </AlertDialogHeader>
          <AlertDialogFooter>
