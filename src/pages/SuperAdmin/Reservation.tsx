...
import { 
  Calendar, Users, Clock3, Menu, ChevronLeft, Phone, Store, UserRound
} from "lucide-react";
import { 
  getSuperAdminReservations
} from "../../api/reservation";
import type { ReservationsResponse, ReservationItem } from "../../api/reservation";
    ...
    data.stores.forEach(store => {
      if (selectedStoreId === null || selectedStoreId === store.store_id) {
        store.reservations.forEach(res => {
          list.push({ storeName: store.store_name, storeId: store.store_id, res });
        });
      }
    });
    // Sort by created_at descending
    return list.sort((a, b) => new Date(b.res.created_at).getTime() - new Date(a.res.created_at).getTime());
    ...
    <div className="p-6 space-y-6 max-w-7xl mx-auto">
      ...
      <div className="flex flex-col md:flex-row md:items-center justify-between gap-4">
        <div className="flex items-center gap-3">
          <Button variant="outline" size="icon" className="h-8 w-8 rounded-full hidden md:flex">
            <ChevronLeft className="h-4 w-4" />
          </Button>
          <div>
            <h1 className="text-3xl font-bold tracking-tight">Reservations</h1>
            <p className="text-muted-foreground text-sm">View and manage store reservations</p>
          </div>
        </div>
        ...
        <div className="flex flex-wrap items-center gap-3">
          ...
          <div className="flex items-center gap-2 bg-card p-1 rounded-md border shadow-sm">
            ...
            <Input 
              ...
              className="border-none shadow-none focus-visible:ring-0 h-8 w-auto bg-transparent"
            />
          </div>
        ...
        <div className="space-y-6">
          ...
          {data.stores.length > 0 && (
            <div className="flex overflow-x-auto pb-4 gap-5 scrollbar-hide snap-x">
              {data.stores.map((store) => (
                <Card 
                  key={store.store_id} 
                  onClick={() => setSelectedStoreId(selectedStoreId === store.store_id ? null : store.store_id)}
                ...
                >
                  <CardContent className="p-5 flex flex-col justify-between h-full">
                    <div>
                      ...
                      <div className="font-bold text-xl">{store.store_name}</div>
                    </div>
                    
                    <div className="mt-5 pt-4 border-t flex justify-between text-base">
                      <span className="text-muted-foreground">Total: <strong className="text-foreground">{store.total}</strong></span>
                      <span className="text-muted-foreground">Booked: <strong className="text-foreground">{store.booked}</strong></span>
                    </div>
                  </CardContent>
                </Card>
              ))}
          ...
          <div className="grid grid-cols-1 lg:grid-cols-2 gap-6 pb-12">
            {allReservations.map(({storeName, storeId, res}) => (
              <Card key={`${storeId}-${res.id}`} className="overflow-hidden flex flex-col shadow-sm hover:shadow-md transition-shadow">
                <CardContent className="p-0 flex flex-col h-full">
                    ...
                    <div className="flex justify-between items-start mb-2">
                      ...
                      <div className="flex flex-col items-end gap-1.5">
                        <div className="flex items-center gap-1.5 text-muted-foreground text-sm font-medium">
                          ...
                          <span>{format(new Date(res.created_at), "h:mm a")}</span>
                        </div>
                        <span className="text-sm font-mono text-muted-foreground bg-muted px-2 py-0.5 rounded-md">#{res.id}</span>
                      </div>
                    </div>
                  </div>
                    ...
                    <div className="grid grid-cols-2 gap-y-4 gap-x-6 text-base">
                      ...
                      <div className="flex flex-col gap-1">
                        ...
                        <span className="font-medium flex items-center gap-1.5">
                          ...
                          {format(new Date(res.reserved_for), "dd MMM yy, h:mm a")}
                        </span>
                      </div>
                  ...
                  <div className="p-5 pt-0 mt-auto">
                    <Button variant="default" className="w-full font-semibold h-11 text-base">
                      Vorbestellen
                    </Button>
                  </div>
                </CardContent>
            ...
            {allReservations.length === 0 && (
              <div className="col-span-full py-16 flex flex-col items-center justify-center text-center border-2 border-dashed rounded-xl bg-card/50">
                ...
                <p className="text-muted-foreground">Try selecting a different date or store.</p>
              </div>
            )}
          </div>
        </div>
