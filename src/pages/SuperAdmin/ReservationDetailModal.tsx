...
import { getReservationDetails, type ReservationDetail } from "../../api/reservation";
...
export const ReservationDetailModal: React.FC<ReservationDetailModalProps> = ({ reservationId, onClose }) => {
  ...
  return (
    <Dialog open={!!reservationId} onOpenChange={(open) => !open && onClose()}>
      <DialogContent className="max-w-md p-0 overflow-hidden sm:rounded-2xl rounded-xl">
        <DialogDescription className="sr-only">Reservation Details</DialogDescription>
        ...
        {!loading && !error && data && (
          <div className="flex flex-col h-full">
            ...
            <div className="flex items-center gap-3 p-4 bg-background">
              <button onClick={onClose} className="p-1 hover:bg-accent hover:text-accent-foreground rounded-full transition-colors">
            ...
            <div className="p-5 flex-1 space-y-5 bg-background">
              ...
              <div className="flex flex-col items-center justify-center text-center gap-1">
                <h3 className="text-lg font-medium text-foreground">Order ID: {data.id}</h3>
              ...
              <div className="space-y-2.5">
                <div className="flex gap-2">
                  <span className="text-foreground font-medium w-24">Customer</span>
                  <span className="text-foreground">: {data.customer_name}</span>
                </div>
                <div className="flex gap-2">
                  <span className="text-foreground font-medium w-24">Phone</span>
                  <span className="text-foreground">: {data.customer_phone || "-"}</span>
                </div>
                <div className="flex gap-2">
                  <span className="text-foreground font-medium w-24">Guest</span>
              ...
              <div className="flex gap-2">
                ...
                <span className="text-foreground">{format(new Date(data.reserved_for), "dd-MM-yyyy HH:mm")}</span>
              </div>
